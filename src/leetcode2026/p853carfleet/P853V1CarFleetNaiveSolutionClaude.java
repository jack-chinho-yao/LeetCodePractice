...
public class P853V1CarFleetNaiveSolutionClaude {
    ...
    public int carFleet(int target, int[] position, int[] speed) {
        ...
        for (double time : positionTime.descendingMap().values()) {
            if (time > lead) {   // arrives later than the fleet ahead → can't catch it → new fleet
                fleets++;
                lead = time;
            }
        }
    ...
    public static void main(String[] args) {
        P853V1CarFleetNaiveSolutionClaude solution = new P853V1CarFleetNaiveSolutionClaude();
