...
public class P146V2LRUCacheOptimizedSolutionPractice002 {
    ...
    class LRUCache{
        ...
        public void put(int key, int value){
            ...
            if (node != null){
                ...
                node.value = value;
                moveToTail(node);
            }else if (capacity == keyNodeMap.size()){
                Node oldest = head.next;
                keyNodeMap.remove(oldest.key);
                removeNode(oldest);
    ...
    public static void main(String[] args) {
        LRUCache cache = new P146V2LRUCacheOptimizedSolutionPractice002().new LRUCache(2);
        cache.put(1, 1);
        cache.put(2, 2);
        cache.put(3, 3);                   // 滿了 → evict 最舊的 key 1
        cache.get(2);                       // 存取 2 → 2 變成 MRU，此時 LRU 應該是 3
        cache.put(4, 4);                   // 滿了 → 正確應該 evict key 3（LRU）
        System.out.println(cache.get(3));  // 你的程式印: ?   正確應該: -1（3 被踢掉了）
        System.out.println(cache.get(2));  // 你的程式印: ?   正確應該: 2（2 剛被存取過，還在）
    }
}
