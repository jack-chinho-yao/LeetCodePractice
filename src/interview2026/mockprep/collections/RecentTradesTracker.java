 ...
 * ============================================================================
 */
public class RecentTradesTracker {
    public final Integer LIMIT;
    public final Map<String, LinkedList<String>> map;

    public RecentTradesTracker(Integer limit){
        if (limit > 0){
            LIMIT = limit;
            map = new HashMap<>();
        }else {
    ...
    public void recordTrade(String traderId, String tradeId){
        LinkedList<String> tradeIds = map.get(traderId);
        if (tradeIds == null){
            map.put(traderId, new LinkedList<>(Arrays.asList(tradeId)));
        }else {
            tradeIds.add(tradeId);
            if (tradeIds.size() > LIMIT){
                tradeIds.remove(0);
            }
        }
    }
