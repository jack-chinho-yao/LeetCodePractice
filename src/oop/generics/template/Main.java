...
//        System.out.println("copied = " + copyOut); // copied = [3, 1, 4, 1, 5, 9, 2, 6]
        ...
        //      invariance —— the line below will NOT compile, know why?
        // List<Number> bad = nums; // compile error

        // G9: 型別擦除 —— 執行期型別參數消失，兩個 Box 是同一個 class
        //     type erasure —— the type argument vanishes at runtime; the two Boxes are the same class
...
/*
    ...
    Checklist of generics concepts to practise  —  details at the bottom of ../example/Main.java
    ============================================================================
    ...
    [ ] G5   wildcard ?               (出現在 G6/G7 / appears in G6/G7)
    [ ] G6   ? extends (producer/讀 read)   Util.sum
    [ ] G7   ? super  (consumer/寫 write)   Util.fillWithOneToThree
    [ ] PECS Producer-Extends         Util.copy
             Consumer-Super
    [ ] G8   generic interface        Container<T> / ListContainer<T>
    [ ] G9   type erasure             getClass() == getClass() // true
    [ ] G10  invariance               List<Integer> 不是 List<Number> (is NOT a List<Number>)

    寫完後在 src/ 下編譯執行：
    When done, compile & run from src/:
      javac oop/generics/template/*.java
      java  oop.generics.template.Main
    輸出要與 example 版一致。
    The output should match the example version.
*/
